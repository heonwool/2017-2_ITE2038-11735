...
SELECT P.name, COUNT(*) FROM Pokemon P JOIN CatchedPokemon C ON P.id = C.pid GROUP BY C.pid ORDER BY COUNT(*) DESC;
...
SELECT T.name, P.type, COUNT(P.type) FROM Trainer T JOIN CatchedPokemon C ON T.id = C.owner_id JOIN Pokemon P ON C.pid = P.id GROUP BY T.name, P.type ORDER BY T.name, P.type, COUNT(P.type);
SELECT T.name AS 'Trainer', P.name AS 'Pokemon', COUNT(P.id) AS '#Captured' FROM Trainer T JOIN CatchedPokemon C ON T.id = C.owner_id JOIN Pokemon P on P.id = C.pid GROUP BY T.name HAVING COUNT(DISTINCT P.id) = 1;
...
SELECT C.pid FROM CatchedPokemon C JOIN Trainer T ON T.id = C.owner_id AND T.hometown IN ('Sangnok City') WHERE C.pid IN (SELECT C.pid from CatchedPokemon C JOIN Trainer T ON T.id = C.owner_id AND T.hometown IN ('Blue City'));
SELECT E1.after_id FROM Evolution E1 WHERE NOT EXISTS (SELECT E2.before_id FROM Evolution E2 WHERE E1.after_id = E2.before_id);
